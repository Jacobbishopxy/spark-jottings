...
object Job5 {
  ...
  lazy val queryFromLastUpdate = (time: Any) => s"""
  SELECT * FROM $sourceTable WHERE $timeColumn > '$time'
  """

  private def runJdbcDatasetUpdateByLastTime(conn: Conn)(implicit spark: SparkSession): Unit = {
    log.info("Starting runJdbcDatasetUpdateByLastTime...")
    ...
    log.info("Task runJdbcDatasetUpdateByLastTime complete!")
  }
}
